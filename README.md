...
- ✍️ **Prompt editor:** one empty free-text field for every mode; Insert Prompt Structure adds the former structured template on demand. Insert [Shot N] and Insert RefMod # use small button-adjacent number inputs; Prefill Labels & Summary uses active reference kinds, descriptions and audio tracks. Prompt Forge lets you review generated text before applying it. Old structured workflows and reference packs migrate into the same field.
